...
export default function PdfsPage() {
    ...
    const filteredPdfs = pdfs.filter(pdf => {
        ...
        if (isAdmin && filterDistributor && filterDistributor !== '') {
            // Check if this PDF is assigned to the filtered distributor
            if (pdf.assignedGroup === 'ALL') {
                // 'ALL' PDFs match any distributor filter
                ...
                // when a distributor filter is active (this is a limitation)
                matchesDistributor = true;
            }
        }
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="border border-gray-300 rounded-lg max-h-64 overflow-y-auto bg-white shadow-sm">
                                                {distributors.map((dist) => (
                                                    <label
                                                        key={dist.id}
                                                        className={`flex items-center gap-3 p-3 cursor-pointer transition-all border-b border-gray-100 last:border-b-0 ${selectedDistributors.includes(dist.id)
                                                    ...
                                                    >
                                                        ...
                                                        />
                                                        <div className="flex-1 min-w-0">
                                                            <p className={`font-medium truncate ${selectedDistributors.includes(dist.id) ? 'text-orange-900' : 'text-gray-900'
                                                                }`}>
                                                                {dist.companyName}
                                                            </p>
                        ...
                        {isAdmin && (
                            <div className="relative">
                                <select
                                    ...
                                    className={`pl-4 pr-10 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none w-full md:w-48 appearance-none transition-all bg-white ${filterDistributor ? 'border-orange-500 bg-orange-50 font-medium text-orange-900' : 'border-gray-300'
                                        }`}
                                >
                                ...
                                <Building2 className={`w-4 h-4 absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none ${filterDistributor ? 'text-orange-500' : 'text-gray-400'
                                    }`} />
                                {filterDistributor && (
                                    <button

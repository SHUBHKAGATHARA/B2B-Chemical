...
export default function PdfsPage() {
    ...
    const loadData = async () => {
        try {
            ...
            if (isAdminUser) {
                ...
                // Load categories with retry logic (database might be waking up)
                ...
                // Load categories for distributor view too
                await loadCategories(token);
            }
        } catch (error) {
            console.error('[PDF Page] Failed to load data:', error);
    ...
    // Separate function to load categories with retry logic
    const loadCategories = async (token: string | null, retryCount = 0) => {
        const maxRetries = 3;
        const retryDelay = 2000; // 2 seconds

        try {
            console.log(`[PDF Page] Loading categories (attempt ${retryCount + 1}/${maxRetries + 1})...`);
            
            const response = await fetch('/api/pdf-categories', {
                headers: {
            ...
            console.log('[PDF Page] Categories response:', categoriesResponse);

            // Handle different response formats
            if (categoriesResponse.success && categoriesResponse.data) {
                setCategories(categoriesResponse.data);
                console.log('[PDF Page] Categories loaded successfully:', categoriesResponse.data.length);
            } else if (Array.isArray(categoriesResponse)) {
                setCategories(categoriesResponse);
                console.log('[PDF Page] Categories loaded as array:', categoriesResponse.length);
            } else if (categoriesResponse.error?.code === 'DB_CONNECTION_ERROR' && retryCount < maxRetries) {
                // Database connection error - retry after delay
                console.log(`[PDF Page] Database connection error, retrying in ${retryDelay}ms...`);
                await new Promise(resolve => setTimeout(resolve, retryDelay));
                return loadCategories(token, retryCount + 1);
            } else {
                console.warn('[PDF Page] Categories response format unexpected:', categoriesResponse);
                setCategories([]);
            }
        } catch (error) {
            console.error('[PDF Page] Error loading categories:', error);
            
            // Retry on network errors
            if (retryCount < maxRetries) {
                console.log(`[PDF Page] Retrying in ${retryDelay}ms...`);
                await new Promise(resolve => setTimeout(resolve, retryDelay));
                return loadCategories(token, retryCount + 1);
            } else {
                console.error('[PDF Page] Max retries reached, giving up');
                setCategories([]);
            }
        }
    };
    ...
    const handleUpload = async (e: React.FormEvent | React.MouseEvent) => {
        ...
        // Validation
        if (!selectedCategory) {
            alert('Please select a PDF category');
            return;
        }
    ...
    // Filter PDFs based on search term, category, and distributor
    const filteredPdfs = pdfs.filter(pdf => {
        ...
        if (isAdmin && filterDistributor) {
            if (pdf.assignedGroup === 'ALL') {
                matchesDistributor = true; // ALL distributors includes the filtered one
            } else if (pdf.assignedGroup === 'SINGLE') {
                matchesDistributor = pdf.distributor?.id === filterDistributor;
            } else {
                // For MULTIPLE, we'd need to check notifications table
                // For now, show it if distributor matches
                matchesDistributor = pdf.distributor?.id === filterDistributor;
            }
        }
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-8 mb-6">
                    {!file ? (
                        ...
                        <div className="space-y-6">
                            <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 transition-all animate-in fade-in slide-in-from-top-4">
                            ...
                            {/* PDF Category and Description - Now shown first */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {/* PDF Category Dropdown */}
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                                        PDF Category <span className="text-orange-500">*</span>
                                    </label>
                                    <select
                                    ...
                                    {categories.length === 0 && !loading && (
                                        <p className="mt-1 text-xs text-amber-600">
                                            ⚠️ Categories are loading... If this persists, the database may be waking up. Please wait a moment.
                                        </p>
                                    )}
                                    {categories.length > 0 && (
                                        <p className="mt-1 text-xs text-gray-500">Categorize this document for better organization ({categories.length} categories available)</p>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    />
                                    <p className="mt-1 text-xs text-gray-500">Provide additional context about this document</p>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {filteredPdfs.length !== pdfs.length && (
                            <span className="text-sm text-gray-600 bg-gray-100 px-3 py-1 rounded-full">
                                {filteredPdfs.length} of {pdfs.length} transfers
                            </span>
                        )}
                    ...
                    <div className="flex flex-col md:flex-row gap-3">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none w-full md:w-48"
                            />
                        </div>
                        ...
                        <select
                            ...
                            className="px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none w-full md:w-44"
                        >
                        ...
                        {isAdmin && (
                            <select
                                ...
                                className="px-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none w-full md:w-48"
                            >
